...
# für US-Version:
# CDEFS += -DUSTTY
# CDEFS += -DBIT_LENGTH=22
# CDEFS += "-DPROG_ID_ZUSATZ=\"USTTY 45 Baud\""
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
CFLAGS += -D__heap_end=0x80ffff -D$(HARDWARE) -D$(ADDON) -D__PROG_TYPES_COMPAT__
...
ELFSIZE = $(SIZE) --mcu=$(MCU) $(TARGET).elf
...
sizeafter:
#	@if test -f $(TARGET).elf; then echo; echo $(MSG_SIZE_AFTER); $(ELFSIZE) -C ; \
#	2>/dev/null; echo; fi
...
#	mode com1: BAUD=9600 PARITY=N DATA=8 STOP=1 to=off xon=off odsr=off octs=off dtr=on rts=on idsr=off
	update/fboot.exe /C1 /B115200 /P$(TARGET).hex /V$(TARGET).hex
