...
//	STDOUT_set( TAFEL, 0);
	...
	printf_P( config_ok );
	// LED_core starten
	// Callbackfunktion fÃ¼r die blinkende LED eintragen
#if !defined(iTelex)
...
// LCD initialisieren wenn in Config angewÃ¤hlt und wenn wir auf einen myEthernet laufen
#if defined(LCD)
		LCD_init( 0 );
...
#else
	#if defined(myAVR)
		// muss auf High stehen, sonst kann es sein wenn MMC nicht aktiv ist und eine MMC steckt das der SPI-Bus blÃ¶dsinn macht
		DDRC |= (1<<PC6);
		PORTC |= (1<<PC6);
	#endif
#endif
